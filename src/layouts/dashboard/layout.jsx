...
/**
 * Mapeia organizações do login para o formato do popover (id, name, logo?, plan?).
 * A API pode retornar { id, name, logo?, plan?, slug?, ... }.
 */
function mapOrganizationsToWorkspaces(organizations) {
  if (!Array.isArray(organizations) || organizations.length === 0) return [];
  return organizations.map((org) => ({
    id: org.id ?? org.slug ?? String(org.name),
    name: org.name ?? 'Organization',
    logo: org.logo ?? `${CONFIG.site.basePath}/assets/icons/workspaces/logo-1.webp`,
    plan: org.plan ?? null,
  }));
}
...
export function DashboardLayout({ sx, children, data }) {
  ...
  const workspaces = useMemo(() => mapOrganizationsToWorkspaces(organizations), [organizations]);
