...
function isPathInChildren(pathname, children) {
  if (!children?.length) return false;
  const normalized = removeLastSlash(pathname);
  for (const item of children) {
    if (removeLastSlash(item.path) === normalized) return true;
    if (item.children && isPathInChildren(pathname, item.children)) return true;
  }
  return false;
}
...
export function NavList({ data, render, depth, slotProps, enabledRootRedirect }) {
  ...
  const activeByPath = useActiveLink(data.path, !!data.children);
  const activeByChild = data.children ? isPathInChildren(pathname, data.children) : false;
  ...
  if (data.children) {
    return (
      ...
      >
        ...
        <NavCollapse data-group={data.title} in={openMenu} depth={depth}>
          <NavSubList
            data={data.children}
            render={render}
            depth={depth}
