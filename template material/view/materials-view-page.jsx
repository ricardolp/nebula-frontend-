...
export function MaterialsViewPageComponent({ id }) {
  ...
  useEffect(() => {
    const fetchLogs = async () => {
      if (!logsPopupOpen || !id) return;

      try {
        setLogsLoading(true);
        const response = await axios.get(`/api/materials/${id}/logs`);
  ...
  const handleSendToSap = useCallback(async () => {
    ...
    try {
      ...
      const result = await sendMaterialToSap(id);
      ...
      <Grid item xs={12} md={6}>
        <Card sx={{ p: 3 }}>
          ...
          <Stack spacing={2}>
            ...
            {renderField('Setor Industrial', materialData?.ind_sector)}
